...
namespace UniquomeApp.WebApi.Controllers.V1
{
    public class IdentityController : BaseApiController
    {
        ...
        [HttpPost(ApiRoutesV1.Identity.RegisterSecretariat)]
        public async Task<IActionResult> RegisterSecretariat([FromBody] UserRegistrationRequest request)
        {
            if (!ModelState.IsValid)
                return BadRequest(
                    AuthenticationResult.AuthFailure(
            ...
            var alteredRequest = GetRegisterRequest(request.Email, request.Password, request.ConfirmPassword,
                AuthorizationConstants.Roles.Secretariat);
            var result = await PerformRegistration(alteredRequest);
            return !result.Succeeded ? (IActionResult)BadRequest(result) : Ok(result);
        }

        [HttpPost(ApiRoutesV1.Identity.RegisterScientificDirector)]
        public async Task<IActionResult> RegisterScientificDirector([FromBody] UserRegistrationRequest request)
        {
            if (!ModelState.IsValid)
                return BadRequest(
                    AuthenticationResult.AuthFailure(
            ...
            var alteredRequest = GetRegisterRequest(request.Email, request.Password, request.ConfirmPassword,
                AuthorizationConstants.Roles.ScientificDirectors);
            var result = await PerformRegistration(alteredRequest);
            return !result.Succeeded ? (IActionResult)BadRequest(result) : Ok(result);
        }

        [HttpPost(ApiRoutesV1.Identity.RegisterCommitteeMember)]
        public async Task<IActionResult> RegisterCommitteeMember([FromBody] UserRegistrationRequest request)
        {
            if (!ModelState.IsValid)
                return BadRequest(
                    AuthenticationResult.AuthFailure(
                        ModelState.Values.SelectMany(x => x.Errors.Select(xx => xx.ErrorMessage))));
            var alteredRequest = GetRegisterRequest(request.Email, request.Password, request.ConfirmPassword,
                AuthorizationConstants.Roles.CommitteeUser);
            var result = await PerformRegistration(alteredRequest);
            return !result.Succeeded ? (IActionResult)BadRequest(result) : Ok(result);
        }

        [HttpPost(ApiRoutesV1.Identity.RegisterCooperator)]
        public async Task<IActionResult> RegisterCooperator([FromBody] UserRegistrationRequest request)
        {
            if (!ModelState.IsValid)
                return BadRequest(
                    AuthenticationResult.AuthFailure(
                        ModelState.Values.SelectMany(x => x.Errors.Select(xx => xx.ErrorMessage))));
            var alteredRequest = GetRegisterRequest(request.Email, request.Password, request.ConfirmPassword,
                AuthorizationConstants.Roles.Cooperators);
            var result = await PerformRegistration(alteredRequest);
            return !result.Succeeded ? (IActionResult)BadRequest(result) : Ok(result);
        }
